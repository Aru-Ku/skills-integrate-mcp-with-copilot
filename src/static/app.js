document.addEventListener("DOMContentLoaded", () => {
  ...
  // Helper: Assign categories to activities (stub for now)
  function getCategory(name) {
    const sports = ["Soccer Team", "Basketball Team", "Gym Class"];
    const arts = ["Art Club", "Drama Club"];
    const academic = ["Math Club", "Programming Class", "Chess Club", "Debate Team"];
  ...
  function renderActivities() {
    ...
    if (category) {
      activities = activities.filter(([name]) => getCategory(name) === category);
    }
    // Sort
    ...
    if (sort === "name") {
      ...
      activities.sort((a, b) => {
        const aLeft = a[1].max_participants - a[1].participants.length;
        const bLeft = b[1].max_participants - b[1].participants.length;
        return bLeft - aLeft;
      });
    }
    // Render
  ...
  // Event listeners for filters/sort/search
